...
## Coordination Task Visualisation

**[▶ View live simulation](https://claude.ai/code/artifact/e57b2cff-e6ca-491c-a49a-503931999725)** — interactive animations of both coordination tasks (Commons Dilemma and Information Consensus), showing the difference between coordinated and failing outcomes in real time.

## Quick Start (Local, Python 3.10)
Use this section if you want the most practical local run flow for this repository.
...
If you already keep your OpenAI key in a `.env` file somewhere else on your machine, you can load it directly:

```bash
set -a
source /path/to/your/.env
set +a
export KEY_OWNER="your_name"
```

If you prefer to export variables manually instead:
...
Optional model overrides that work better with current OpenAI APIs:
...
### 6. Start the backend simulation server
Open Terminal B:
...
### 7. Create a new simulation run
When the backend prompts:

`Enter the name of the forked simulation:`

Use one of these:
- `base_the_ville_isabella_maria_klaus` for the 3-agent version
- `base_the_ville_n25` for the 25-agent version

Then it will ask:

`Enter the name of the new simulation:`

Type a new simulation name that does not already exist, for example:
...
Important:
- the forked simulation must already exist in `environment/frontend_server/storage`
- the new simulation name must be different from the fork name

### 8. Open the simulator page
In your browser, open:

`http://127.0.0.1:8000/simulator_home`

### 9. Run the simulation
Back in the backend terminal, once you see:
...
If that works, continue with larger batches:
...
Notes:
- do not type just `10` or `100`; the command must be `run 10` or `run 100`
- refresh the browser after the backend returns to `Enter option:`
- large batches can take a long time or stall, so smaller runs are more reliable

### 10. Save your progress
At `Enter option:` you can use:

- `save` to save progress and keep working
- `fin` to save and finish
- `exit` to exit without saving

Recommended:
- use `save` often during long runs
- if a batch appears stuck, press `Ctrl+C`, wait for `Enter option:`, then run `save`

### 11. Replay a saved simulation later
Keep the frontend server running and open:

`http://127.0.0.1:8000/replay/<simulation-name>/1/`

Replace `<simulation-name>` with the folder name of the saved run.

### 12. Common problems
`Please start the backend first`

- the backend server is not running, crashed, or has not reached `Enter option:`

`FileExistsError`

- your new simulation name already exists; choose a different name

`FileNotFoundError` for a forked simulation

- the forked simulation folder name does not exist under `environment/frontend_server/storage`

The run seems frozen for several minutes

- this codebase can stall on long LLM-heavy batches
- interrupt with `Ctrl+C`
- wait for `Enter option:`
- continue with a smaller batch such as `run 10` or `run 50`
