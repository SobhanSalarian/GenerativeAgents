...
from llm_usage import get_usage_stats, reset_usage_stats
...
class ReverieServer: 
  def __init__(self, 
    ...
    available_personas = list(reverie_meta.get("persona_names", []))
    selected_personas = list(persona_names) if persona_names else available_personas
    missing_personas = [name for name in selected_personas
                        if name not in available_personas]
    if missing_personas:
      raise ValueError(
        f"Requested personas not present in fork '{fork_sim_code}': "
        f"{missing_personas}"
      )
    ...
    with open(f"{sim_folder}/reverie/meta.json", "w") as outfile: 
      ...
      reverie_meta["experimental_condition"] = self.experimental_condition.name
      reverie_meta["persona_names"] = self.selected_personas
      reverie_meta["run_seed"] = self.run_seed
    ...
    persona_root = f"{sim_folder}/personas"
    for persona_name in available_personas:
      if persona_name in self.selected_personas:
        continue
      remove_path = f"{persona_root}/{persona_name}"
    ...
    # <scenario> holds an optional BaseScenario instance. When set, its
    # step() method is called automatically after every simulation step
    # inside start_server(). Set via rs.scenario = <your_scenario>.
    self.scenario = None
  ...
  def get_run_manifest(self):
    """
    Return lightweight metadata about the current experimental run.
    """
    return {
      "fork_sim_code": self.fork_sim_code,
      "sim_code": self.sim_code,
      "experimental_condition": self.experimental_condition.name,
      "selected_personas": list(self.selected_personas),
  ...
  def start_server(self, int_counter):
    """
    Headless backend server of Reverie (no frontend required).
    Each step the server generates its own environment file from the current
    persona tile positions, runs the full cognitive cycle for every persona,
    advances the tile positions to where each persona moves next, then writes
    a movement file as a log.
    INPUT
    ...
    while (True):
      ...
      # --- HEADLESS: self-generate the environment file from current tiles ---
      curr_env_file = f"{sim_folder}/environment/{self.step}.json"
      if not check_if_file_exists(curr_env_file):
        headless_env = {name: {"x": tile[0], "y": tile[1]}
                        for name, tile in self.personas_tile.items()}
        with open(curr_env_file, "w") as f:
          json.dump(headless_env, f)
      ...
      # Clean up object events from the previous cycle.
      for key, val in game_obj_cleanup.items():
        self.maze.turn_event_from_tile_idle(key, val)
      game_obj_cleanup = dict()

      # Move personas on the backend tile map to their current positions.
      for persona_name, persona in self.personas.items():
      ...
      # Run the full cognitive cycle for every persona.
      ...
      for persona_name, persona in self.personas.items():
        ...
        # --- HEADLESS: advance persona tile to the next step position ---
        self.personas_tile[persona_name] = tuple(next_tile)

      movements["meta"]["curr_time"] = (
        self.curr_time.strftime("%B %d, %Y, %H:%M:%S"))

      # Write movement log (useful for replay / debugging).
      curr_move_file = f"{sim_folder}/movement/{self.step}.json"
      with open(curr_move_file, "w") as outfile:
        outfile.write(json.dumps(movements, indent=2))

      # Run the attached scenario step (if any) before advancing the clock.
      if self.scenario:
        self.scenario.step(self.personas, self.step, self.curr_time)
  ...
  def open_server(self):
    ...
    print ("Running in headless mode — no frontend server required.\n---")
