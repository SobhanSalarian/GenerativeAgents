...
def generate_poig_score(persona, event_type, description): 
  def _coerce_poignancy(result, default=4):
    # Normalize helper outputs into a bounded integer score so perception
    # keeps running even if the prompt layer returns an unexpected shape.
    if result is None:
      return default
  ...
  if event_type == "event": 
    return _coerce_poignancy(
      run_gpt_prompt_event_poignancy(persona, description))
  elif event_type == "chat": 
    return _coerce_poignancy(
      run_gpt_prompt_chat_poignancy(persona, 
                           persona.scratch.act_description))
  return 4

def perceive(persona, maze, store_to_memory=True): 
  """
  ...
  When store_to_memory is False, the persona still perceives nearby events but
  does not append them to associative memory. This supports the thesis baseline
  condition, which should not silently accumulate episodic traces.
  """
  # PERCEIVE SPACE
