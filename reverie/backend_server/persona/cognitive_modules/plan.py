...
def generate_wake_up_hour(persona):
  ...
  return int(result[0]) if result else 8
...
def generate_task_decomp(persona, task, duration): 
  ...
  return result[0] if result else [[task, duration]]
...
def generate_action_event_triple(act_desp, persona): 
  ...
  return result[0] if result is not None else (persona.scratch.name, "is", "idle")
...
def generate_act_obj_event_triple(act_game_object, act_obj_desc, persona):
  ...
  return result[0] if result is not None else (act_game_object, "is", "idle")
...
def generate_convo_summary(persona, convo):
  ...
  return result[0] if result else "conversing"
...
def generate_decide_to_react(init_persona, target_persona, retrieved):
  ...
  return result[0] if result else "wait"
...
def _determine_action(persona, maze): 
  ...
  # Some forks can start with an empty persisted schedule (e.g., manually
  # seeded times). Ensure we always have a valid action slot.
  if not persona.scratch.f_daily_schedule:
    minutes_elapsed = persona.scratch.curr_time.hour * 60 + persona.scratch.curr_time.minute
    minutes_remaining = max(1, 1440 - minutes_elapsed)
    persona.scratch.f_daily_schedule = [["sleeping", minutes_remaining]]
    curr_index = 0
    curr_index_60 = 0
